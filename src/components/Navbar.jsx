...
export default function Navbar() {
    ...
    return <nav className="fixed top-0 z-50 w-full h-16 transition-all duration-300 bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-200">
                ...
                <div className="md:hidden absolute top-16 left-0 w-full bg-white border-b border-gray-200 shadow-xl py-6 px-4 space-y-4 flex flex-col animate-in slide-in-from-top duration-300">
                    {[{name: 'Home', path: '/'}, {name: 'About', path: '/about'}, {name: 'Student', path: '/student'}, {name: 'Teacher', path: '/teacher'}, {name: 'Notices', path: '/notices'}].map((item) => (
                        <Link
                            key={item.name}
                            to={item.path}
                            className={`font-semibold text-lg border-b border-gray-100 pb-2 hover:text-blue-600 ${location.pathname === item.path ? 'text-blue-600' : 'text-gray-800'}`}
